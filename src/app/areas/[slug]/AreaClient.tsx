        ...
        <div className={styles.section}>
          ...
          <div className={styles.headRow}>
            ...
            {/* Was "Open the full planner" -> useThisDay(), duplicating the
                ...
                testing feedback): relabelled "See all days", pointed at a
                real Days list sorted by proximity to this Area instead of
                performing the same action twice.
                Revised again on the main merge (10 Aug 2026): the first
                version of this fix built its own one-off ?near= proximity
                sort against the old flat /days page. The days-trip-flow
                branch's Phase 2 rebuild (merged into main first) already
                has a real, more general version of exactly this - Trip
                Answers' base/baseKind drives DaysHubGrid's own drive-time
                grouping (day-derivations.ts's driveMinutesForDay), so
                setting this Area as the visitor's base and landing on the
                plain Days hub gets the same "sorted near here" outcome
                through the real system, not a parallel one-off. */}
            <Link
              href="/days"
              className={styles.sectionLink}
              onClick={() => trip.setAnswersBase(a.slug, "area", { name: a.name, lat: a.lat, lng: a.lng })}
            >
              See all days →
            </Link>
          </div>
