...
9. Retire/simplify Q2 (LocationStep, region picker) and Q3 (InterestsStep) of the old intake flow — already inactivated on the live site, but the code (and their shared "below-the-fold homepage mirror," `HomeSectionsBelowFold.tsx`) is still there. Mark's own read (11 Aug 2026): these are "mostly redundant now anyway, as we have a new initial question flow" (the hero sentence control). Needs a proper look at what's still reachable through them before touching any code, not a quick removal — flagged, not started.
